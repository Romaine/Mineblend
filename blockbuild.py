...
import os
import sys
from Mineblend.sysutil import MCPATH, MCSAVEPATH
...
def construct(blockID, basename, diffuseRGB, cubeTexFaces, extraData, constructType="box", shapeParams=None, cycParams=None):
    ...
    if constructType == 'box':
        block = createMCBlock(
            basename, diffuseRGB, cubeTexFaces, cycParams)  # extra data
    elif constructType == 'onehigh':
        block = createInsetMCBlock(
            basename, diffuseRGB, cubeTexFaces, [0, 15, 0], cycParams)
    elif constructType == '00track':
        block = createTrack(
            basename, diffuseRGB, cubeTexFaces, extraData, cycParams)
    # elif constructType == 'hash':  #or crop? Is it the same? crops, etc.
    elif constructType == 'cross':
        block = createXBlock(
            basename, diffuseRGB, cubeTexFaces, extraData, cycParams)
    elif constructType == 'stair':
        block = createStairBlock(
            basename, diffuseRGB, cubeTexFaces, extraData, cycParams)
    elif constructType == 'fence':
        block = createFenceBlock(
            basename, diffuseRGB, cubeTexFaces, shapeParams, cycParams)
                                 # for this, shape params will be NESW flags.
    elif constructType == 'inset':  # make an inset box (requires shapeParams)
        block = createInsetMCBlock(
            basename, diffuseRGB, cubeTexFaces, shapeParams, cycParams)  # shapeprms must be a 3-list
    else:
        block = createMCBlock(basename, diffuseRGB, cubeTexFaces, cycParams)
                              # extra data	# soon to be removed as a catch-all!
    return block
...
def getMCImg():
    ...
    if TERRAIN_TEXTURE_ATLAS_NAME in bpy.data.images:
        ...
        if os.path.exists(temppath):
            ...
            # generate a placeholder image for the texture if terrain.png
            # doesn't exist (rather than failing)
            print("no terrain texture found... creating empty")
            img = bpy.data.images.new(
                TERRAIN_TEXTURE_ATLAS_NAME, 1024, 1024, True, False)
            img.source = 'FILE'
        os.chdir(osdir)
        return img
...
def createBMeshBlockCubeUVs(blockname, me, matrl, faceIndices):  # assume me is a cube mesh.  RETURNS **NAME** of the uv layer created.
    ...
    # BMESH loop  face order is [left,back,right,front,bottom,top] (for
    # default cube)
    if DEBUG_BBUV:
        print("createBMeshBlockCubeUVs " + blockname +
              " attempting to get faces: " + str(faceIndices))
    bmfi = [faceIndices[4], faceIndices[5], faceIndices[2],
            faceIndices[3], faceIndices[0], faceIndices[1]]
    ...
    uvcornersTop = [(uvUnit, -uvUnit), (uvUnit, 0.0),
                    (0.0, 0.0), (0.0, -uvUnit)]  # 4,1,2,3
    # bottom is rotated and flipped from default
    uvcornersBot = [(0.0, -uvUnit), (0.0, 0.0),
                    (uvUnit, 0.0), (uvUnit, -uvUnit)]  # 3,2,1,4

    # we have to assign each UV in sequence of the 'loop' for the whole mesh:
    # 24 for a cube.
    ...
    for pface in me.polygons:
        ...
        v1 = 1.0 - \
            v1  # y goes low to high   #DEBUG print("That means u1,v1 is %f,%f" % (u1,v1))
        # DEBUG
        if DEBUG_BBUV:
            print("mcTexU,mcTexV " + str(mcTexU) + ", " +
                  str(mcTexV) + " - u1, v1 %d,%d" % (u1, v1))
            # print("minecraft chunk texture x,y within image: %d,%d" % (mcTexU, mcTexV))
        # if DEBUG_BBUV:
        # print("createBMeshBlockCubeUVs "+blockname+" u1, v1: %d,%d" % (u1,
        # v1))

        loopPolyStart = pface.loop_start  # where its verts start in the loop. Yay!
        # if loop total's not 4, need to work with ngons or tris or do more
        # complex stuff.
        ...
        for uvc in range(loopPolyStart, loopPolyEnd):
            ...
            # apply the calculated face uv + vert offset to the current loop
            # element
    ...
    me.tessface_uv_textures.data.update()
                                        # a guess. does this actually help? YES!
                                        # Without it all the world's grey and
                                        # textureless!
...
def createBlockCubeUVs(blockname, me, matrl, faceIndices):  # assume me is a cube mesh.  RETURNS **NAME** of the uv layer created.
    ...
    uname = blockname + 'UVs'
    blockUVLayer = me.uv_textures.new(
        uname)  # assuming it's not so assigned already, ofc.
    xim = getMCImg()
    meshtexfaces = blockUVLayer.data.values()
    ...
    for fnum, fid in enumerate(faceIndices):
        ...
        u1 = (mcTexU * TEXTURE_ATLAS_PIXELS_PER_UNIT) / \
            TEXTURE_ATLAS_PIXELS    # or >> 4 (div by imagesize to get as fraction)
        v1 = (mcTexV * TEXTURE_ATLAS_PIXELS_PER_UNIT) / \
        ...
        # DEBUG
        if DEBUG_BBUV:
            print("createBlockCubeUVs Creating UVs for face with values: %f,%f to %f,%f" %
                  (u1, v1, mcUV3[0], mcUV3[1]))
...
def createInsetUVs(blockname, me, matrl, faceIndices, insets):
    ...
    for fnum, fid in enumerate(faceIndices):
        ...
        # DEBUG print("Creating UVs for face with values: %f,%f to %f,%f" %
        # (u1,v1,mcUV3[0], mcUV3[1]))
...
def createBMeshInsetUVs(blockname, me, matrl, faceIndices, insets):
    ...
    uvcorners = [(uvUnit - iS, 0.0 - iT), (0.0 + iS, 0.0 - iT),
                 (0.0 + iS, -uvUnit + iB), (uvUnit - iS, -uvUnit + iB)]
    uvcornersTop = [(uvUnit - iS, -uvUnit + iS), (uvUnit - iS, 0.0 - iS),
                    (0.0 + iS, 0.0 - iS), (0.0 + iS, -uvUnit + iS)]  # 4,1,2,3
    uvcornersBot = [(0.0 + iS, -uvUnit + iS), (0.0 + iS, 0.0 - iS),
                    (uvUnit - iS, 0.0 - iS), (uvUnit - iS, -uvUnit + iS)]  # 3,2,1,4
    ...
    for pface in me.polygons:
        ...
        # DEBUG
        if DEBUG_BBUV:
            print("createBMeshInsetUVs minecraft chunk texture x,y within image: %d,%d" %
                  (mcTexU, mcTexV))
    ...
    me.tessface_uv_textures.data.update()
                                        # Without this, all the world is grey
                                        # and textureless!
...
#
# Aside from simplifying the individual material layouts, the reason for
# using Node Groups extensively is to allow for users to easily customize
# the overall look of their scene (i.e. rather than having to modify
# dozens of materials, some changes can have global effect by modifying a
# single Node Group, depending on the change desired)
...
def createNGmcTexture():
    ...
    ng.links.new(imageTex.inputs[0], texCoord.outputs[2])
                 # link the texCoord uv to the imageTex vector
...
def setNodeGroup(node, ngName):
    ...
    # FIXME - is there a better way to use a node group from within a node
    # group?
    node.name = ngName
    node.label = ngName
...
def createNGmcDiffuse():
    ...
    ng.links.new(diffuse.inputs[0], tex.outputs[0])
                 # link the texCoord uv to the imageTex vector
...
def createNGmcDiffuseRGB():
    ...
    # ng.links.new(diffuse.inputs[0],tex.outputs[0]) # link the texCoord uv to
    # the imageTex vector
...
def createNGmcStencil():
    ...
    links.new(input=inNode.outputs[
              MC_GROUP_TEX_OUTPUT], output=rgbtobwNode.inputs['Color'])
    links.new(input=rgbtobwNode.outputs['Val'], output=gtNode.inputs[1])
    links.new(input=gtNode.outputs[
              'Value'], output=mixNode.inputs[FACTOR_INPUT])

    # leave the options open
    links.new(input=inNode.outputs[
              MC_GROUP_TEX_OUTPUT], output=diffNode.inputs['Color'])
...
def createNGmcStencilColored():
    ...
    links.new(input=texNode.outputs[
              MC_GROUP_TEX_OUTPUT], output=rgbtobwNode.inputs['Color'])
    links.new(input=rgbtobwNode.outputs['Val'], output=gtNode.inputs[1])
    links.new(input=gtNode.outputs[
              'Value'], output=alphaMixNode.inputs[FACTOR_INPUT])

    links.new(input=transpNode.outputs[
              BSDF_OUTPUT], output=alphaMixNode.inputs[2])
    ...
    links.new(input=rgbtobwNode.outputs[
              'Val'], output=colorMixNode.inputs[FACTOR_INPUT])
    ...
    links.new(input=colorMixNode.outputs[
              "Color"], output=colorDiffNode.inputs["Color"])
    links.new(input=colorDiffNode.outputs[
              BSDF_OUTPUT], output=alphaMixNode.inputs[1])
...
def createDiffuseCyclesMat(mat):
    ...
    ntree.links.new(
        input=mcdif.outputs[MC_GROUP_DIFFUSE_OUTPUT], output=matOutNode.inputs['Surface'])
...
def createEmissionCyclesMat(mat, emitAmt):
    ...
    emitNode.inputs['Strength'].default_value = float(
        emitAmt)  # set this from the EMIT value of data passed in.
...
def createVegitationCyclesMat(mat):
    ...
    links.new(input=darkColorNode.outputs[
              'Color'], output=stencilNode.inputs[0])
    links.new(input=lightColorNode.outputs[
              'Color'], output=stencilNode.inputs[1])
    links.new(input=stencilNode.outputs[
              'Shader'], output=matNode.inputs['Surface'])
...
def createVegitationCyclesMatOld(mat):
    ...
    links.new(input=diffNode.outputs[
              MC_GROUP_TEX_OUTPUT], output=rgbtobwNode.inputs['Color'])
    links.new(input=rgbtobwNode.outputs['Val'], output=gtNode.inputs[1])
    links.new(input=gtNode.outputs[
              'Value'], output=mixNode.inputs[FACTOR_INPUT])

    # Vegitation difference: feed the matl color into transparent... not
    # needed? FIXME
    links.new(input=transpNode.outputs[BSDF_OUTPUT], output=mixNode.inputs[2])
    ...
    links.new(input=rgbtobwNode.outputs[
              'Val'], output=lcrampNode.inputs[FACTOR_INPUT])
    links.new(input=lcrampNode.outputs[
              'Color'], output=ldiffNode.inputs['Color'])
...
def createPlainAlphaCyclesMat(mat):
    ...
    links = ntree.links
    # FIXME - does reversing order of inputs give the right effect for stained
    # glass?
    links.new(input=diffNode.outputs[BSDF_OUTPUT], output=mixNode.inputs[2])
    links.new(input=diffNode.outputs[
              'Alpha'], output=mixNode.inputs[FACTOR_INPUT])
...
Units are in Minecraft texels - so from 1 to 15. Inset 16 is an error."""
    ...
    mcuvs = None
    if isBMesh():
        mcuvs = createBMeshInsetUVs(
            mcname, mesh, blockMat, mcfaceindices, insets)
    else:
    ...
    bpy.ops.transform.rotate(value=(-1.5708), axis=(0, 0, 1), constraint_axis=(
        False, False, True), constraint_orientation='GLOBAL')
...
# blockMesh.materials.append(blockMat)    # previously is in the uvtex
# creation function for some reason...
    ...
    mcuvs = None
    if isBMesh():
        mcuvs = createBMeshBlockCubeUVs(
            mcname, blockMesh, blockMat, mcfaceindices)
    else:
...
def createFenceBlock(mcname, colourtriple, mcfaceindices, shapeParams, cyclesParams=None):
    ...
    # mcname + "fencePost"
    block = createInsetMCBlock(
        mcname, colourtriple, mcfaceindices, [0, 0, 6], cyclesParams)
...
def createBMeshXBlockUVs(blockname, me, matrl, faceIndices):  # assume me is an X mesh. Returns name of the uv layer created.
    ...
    for faceNo, pface in enumerate(me.polygons):
        ...
        # if loop total's not 4, need to work with ngons/tris or do more complex
        # stuff.
        loopPolyCount = pface.loop_total
        loopPolyEnd = loopPolyStart + loopPolyCount
   ...
   # a guess. does this actually help? YES! Without it all the world's grey
   # and textureless!
    me.tessface_uv_textures.data.update()
...
def createStairsBlock(basename, diffuseRGB, mcfaceindices, extraData, cycParams):
    ...
    # DOES THE FACING DETERMINE THE UV UNWRAP? The public needs to know! if
    # so... nuts! must be easier way? Can do cube mapping and rotate tex
    # space??
    ...
    sverts = [(0.5, 0.5, 0.5),  # v0
              ...
              # would it be a good idea or a bad idea to reverse order of these
              # latter 6?
              ]
    ...
    for v in sverts:
        ...
        # in a stair mesh, we'll have R1,R2 ; stairfacings(vertical)
        # higher,lower; L1,L2; BACK; Top(tip),Top(midstep); Bottom. Maybe.
        # Rearrange for cube order.
        sf1 = stair.faces.new([svs[0], svs[5], svs[4], svs[1]])  # r1
        sf2 = stair.faces.new([svs[4], svs[3], svs[2], svs[1]])  # r2
        sf3 = stair.faces.new(
            [svs[5], svs[11], svs[10], svs[4]])  # vertical topstair face
        sf4 = stair.faces.new(
            [svs[3], svs[9], svs[8], svs[2]])  # vertical bottomstair face
        sf5 = stair.faces.new(
            [svs[9], svs[10], svs[7], svs[8]])  # lface1 (lower..)
        sf6 = stair.faces.new(
            [svs[11], svs[6], svs[7], svs[10]])  # lface2 (upright higher bit)
        sf7 = stair.faces.new([svs[6], svs[0], svs[1], svs[7]])  # back
        sf8 = stair.faces.new(
            [svs[0], svs[6], svs[11], svs[5]])  # topface, topstep
        sf9 = stair.faces.new(
            [svs[4], svs[10], svs[9], svs[3]])  # topface, midstep
...
# nublock3 = createInsetMCBlock("Chest", (164,114,39),
# [25,25,26,27,26,26], [0,1,1])
